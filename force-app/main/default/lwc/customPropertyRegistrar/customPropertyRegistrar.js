...
export default class CustomPropertyRegistrar extends LightningElement {
    ...
    setupEventListeners() {
        ...
        this.template.addEventListener('fieldmapper:ready', this.handlePropertyComponentReady.bind(this));
    ...
    registerCustomProperties() {
        if (!window.Survey || !window.Survey.Serializer) {
            console.error('Cannot register custom properties - Survey.Serializer is not available');
            return;
        }
        ...
        const customPropertyElements = this.template.querySelectorAll('.custom-property');
        
        if (customPropertyElements.length === 0) {
            console.warn('No custom property components found in template');
        } else {
            console.log(`Found ${customPropertyElements.length} custom property components`);
        }
    ...
    registerComponentProperty(element) {
        try {
            ...
            if (!component || typeof component.registerProperty !== 'function') {
                console.error('Component missing registerProperty method:', element.tagName);
                return;
            }
    ...
    handlePropertyComponentReady(event) {
        ...
        try {
            ...
            if (propertyName && !this.registeredProperties.includes(propertyName)) {
                this.registeredProperties.push(propertyName);
                console.log(`Registered custom property from ready event: ${propertyName}`);
            }
        } catch (error) {
