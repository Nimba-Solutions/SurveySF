...
export default class SurveyBuilder extends LightningElement {
  ...
  get currentQuestionText() {
    ...
    const type = this.currentQuestion.getType
      ? this.currentQuestion.getType()
      : "";
  ...
  connectedCallback() {
    ...
    if (urlParams.has("versionId")) {
      ...
      console.log(
        `TODO: Load Survey Content for SurveyVersion__c.Id: ${versionId}`
      );
    } else if (urlParams.has("surveyId")) {
      const surveyId = urlParams.get("surveyId");
      console.log(
        `TODO: Load Survey Content for Survey__c.Id's latest SurveyVersion__c: ${surveyId}`
      );
    }
  }
    ...
    this.initializeSurveyPropertyRegistrar()
      ...
      .then(() => {
        ...
        if (urlParams.has("c__surveyId")) {
          ...
          return loadLatestVersion({ surveyId: this.surveyId }).then(
            (result) => {
      ...
      .catch((error) => {
        ...
        this.showErrorToast(
          "Error loading survey: " + (error.message || JSON.stringify(error))
        );
        this.isLoading = false;
        this.resourcesLoading = false;
      });
  ...
  loadSurveyResources() {
    ...
    return Promise.all([
      ...
      .then(() => {
        console.log(
          "Survey core loaded successfully: window.Survey =",
          !!window.Survey
        );
      ...
      .then(() => {
        console.log(
          "Survey Creator Core loaded successfully: window.SurveyCreator =",
      ...
      .then(() => {
        console.log(
          "Survey Creator loaded successfully: SurveyCreator.SurveyCreator =",
        ...
        if (!window.SurveyCreator || !window.SurveyCreator.SurveyCreator) {
          ...
          console.log(
            "SurveyCreator object keys:",
            window.SurveyCreator
              ? Object.keys(window.SurveyCreator)
              : "undefined"
          );
        }
  ...
  initializeSurvey() {
    ...
    if (
      !window.Survey ||
      !window.Survey.Serializer ||
      !window.SurveyCreator ||
      !window.SurveyCreator.SurveyCreator
    ) {
      console.error(
        "SurveyJS libraries not fully loaded. Survey:",
        !!window.Survey,
        "Serializer:",
        !!window.Survey?.Serializer,
        "SurveyCreator:",
    ...
    try {
      ...
      if (propertyRegistrar) {
        ...
        console.warn(
          "Custom property registrar reference not found for registration"
        );
      }
      ...
      setTimeout(() => {
        ...
        const eventDebugger = this.template.querySelector("c-event-debugger");
        if (eventDebugger) {
          console.log(
            "Refreshing the event debugger with the initialized creator"
          );
      ...
      this.showErrorToast(
        "Error initializing survey: " + (error.message || JSON.stringify(error))
      );
  ...
  checkAndInitializeSurvey() {
    ...
    if (
      this.surveyResourcesLoaded &&
      this.surveyDataLoaded &&
      !this.surveyInitialized
    ) {
      console.log("Both resources and data are loaded, initializing survey...");
      this.initializeSurvey();
    } else {
      console.log(
        "Not ready to initialize yet. Resources loaded:",
        this.surveyResourcesLoaded,
        "Data loaded:",
        this.surveyDataLoaded,
        "Already initialized:",
